...
class TgQ4(Scene):  
    ...
    def construct(self):
        ...
        pontoA=Dot(axes_graph.c2p(np.pi/6,tan(np.pi/6)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        pontoB=Dot(axes_graph.c2p(3*np.pi/4,tan(3*np.pi/4)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        labelB=MathTex("B", 
                       ...
                       font_size=24).next_to(pontoB,
                                             0.25*RIGHT + DOWN,
                                             buff=0.2)
        pontoC=Dot(axes_graph.c2p(4*np.pi/3,tan(4*np.pi/3)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        pontoD=Dot(axes_graph.c2p(7*np.pi/4,tan(7*np.pi/4)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        tangente1=axes_graph.plot(
            lambda x: tan(x),
                   ...
                   discontinuities=[(np.pi / 2)], 
                   color=BLUE)
        tangente2=axes_graph.plot(
            lambda x: tan(x),
                   x_range=[0.2+pi/2,(3*np.pi/2)-0.2],
                   color=BLUE)
        tangente3=axes_graph.plot(
            lambda x: tan(x),
                   x_range=[0.2+3*np.pi/2,(2*np.pi)-0.1],
                   color=BLUE)
        ...
        labelAngle=MathTex("\\frac{7\pi}{4}", 
                       color=WHITE,
        ...
        self.play(Create(curva_parametrica2),Create(tangente1),Create(tangente2),Create(tangente3),run_time=3)
        ...
        def respostas(label,ponto,x):
            labelA=MathTex(label, #O QUE VAI ESTA ESCRITO DO LADO DO PONTO
                       color=WHITE,
                       font_size=24).next_to(ponto, # QUAL O PONTO DA RESPOSTA?
                                             UP+2*RIGHT,#LOCAL EM ERELAÇÃO AO PONTO
                                             buff=0.2) 
            
            linhaRx=DashedLine(start=axes_graph.c2p(x,0),
                        end=axes_graph.c2p(x,tan(x)),
                        color=WHITE)
            linhaRy=DashedLine(start=axes_graph.c2p(0,tan(x)),
                        end=axes_graph.c2p(x,tan(x)),
                        color=WHITE)
        ...
        respostas('D=(\\frac{7\\pi}{4},sen(\\frac{7\\pi}{4}))',pontoA,7*pi/4)        
