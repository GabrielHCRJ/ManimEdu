...
class SenoG2(Scene):     
    ...
    def construct(self):
        ...
        ponto=Dot(axesA.c2p(pi/4,sin(pi/4)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        def graficoAngulo(funcao,angulo,valor):
            graficoR=axes_graph.plot(
                funcao,
                    x_range=[0,angulo],
                    color=RED)
            ponto=Dot(axes_graph.c2p(angulo,sin(angulo)),
                color=RED,
                fill_opacity=5,
                stroke_width=1)
        ...
        self.play(Create(curva_parametricaA),Create(seno),run_time=3)
        ...
        graficoAngulo(lambda x: sin(x),3*pi/4,'\\frac{3\\pi}{4}')
        ...
        circuloB(2,1,0,0,3*pi/4,-0.71,0.71,'B',YELLOW)
