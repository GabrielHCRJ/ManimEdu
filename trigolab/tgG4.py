...
class tgG4(Scene):     
    ...
    def construct(self):
        ...
        tangente2=axes_graph.plot(
            lambda x: tan(x),
                   x_range=[0.8+pi/2,(3*np.pi/2)-0.2],
                   color=BLUE)
        tangente3=axes_graph.plot(
            lambda x: tan(x),
                   x_range=[0.8+3*np.pi/2,(2*np.pi)],
                   color=BLUE)
        ...
        def graficoAngulo(funcao,angulo,valor):
            graficoR=axes_graph.plot(
                funcao,
                    x_range=[0.8+3*pi/2,angulo],
                    color=RED)
            ponto=Dot(axes_graph.c2p(angulo,tan(angulo)),
                color=RED,
                fill_opacity=5,
                stroke_width=1)
        ...
        self.play(Create(curva_parametricaA),Create(tangente1),run_time=2)
        self.play(Create(tangente2),Create(tangente3))
        ...
        graficoAngulo(lambda x: tan(x),5*pi/3+0.5,'\\frac{5\\pi}{3}')
        ...
        circuloB(2,1,0,0,pi/6,0.87,0.5,'B')
        circuloB(-2,-1,0,0,2*pi/3,-0.5,0.87,'C')
        circuloB(2,-1,0,0,5*pi/3,0.5,-0.87,'D',YELLOW)
