...
class SenoQ4(Scene):  
   ...
   #CRIAR NOVOS PONTOS DIFERENTES NO GRÁFICO E A RESPOSTA CERTA VAI SER A LETRA b
    ...
    def construct(self):
        ...
        pontoA=Dot(axes_graph.c2p(2/6*np.pi,sin(2/6*np.pi)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        pontoB=Dot(axes_graph.c2p(3/4*np.pi,sin(3/4*np.pi)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        ...
        self.play(Create(curva_parametrica),run_time=2)      
        ...
        respostas('D=(\\frac{5\\pi}{3},sen(\\frac{5\\pi}{3}))',pontoB,5/3*np.pi)        
