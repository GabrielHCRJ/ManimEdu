...
class SenoDomImg(Scene):
    def construct(self):
        ...
        graph2 = axes.plot((lambda x: sin(x)), x_range=[-10, 10], color=RED)

        #TEXTOS
        mathtext1= MathTex("f(x)=sen(x)").set_color_by_gradient(RED).set_height(0.5)
        domtext= MathTex("Dom: \mathbb{R}")
        imgtext= MathTex("Im: [-1,1]")
