...
class SenoG1(Scene):     
    ...
    def construct(self):
        ...
        pontopi4=Dot(axes_graph.c2p(pi/4,sin(pi/4)),
                   color=RED,
                   fill_opacity=5,
                   stroke_width=1)
        label_pontopi4=MathTex("\\frac{\\pi}{4}",color=WHITE,font_size=24).next_to(pontopi4,0.5*DOWN)
        ...
        curva_parametricaB=axes.plot_parametric_curve(
            lambda t:[np.cos(t),np.sin(t),0],
                   t_range=[0,5*np.pi/4],
                   color=YELLOW)
        ...
        labelAngleA=MathTex("A", 
                       color=RED,
                       font_size=24).next_to(angle,
                                             DOWN + RIGHT,
                                             buff=0.2)
        labelAngleB=MathTex("B", 
                       color=YELLOW,
                       font_size=24).next_to(angle,
                                             DOWN + RIGHT,
                                             buff=0.2)
        labelAngleC=MathTex("C", 
                       color=GREEN,
                       font_size=24).next_to(angle,
                                             DOWN + RIGHT,
                                             buff=0.2)
        labelAngleD=MathTex("D", 
                       color=ORANGE,
                       font_size=24).next_to(angle,
                                             DOWN + RIGHT,
                                             buff=0.2)
        ...
        self.play(Create(curva_parametrica),Create(labelAngleA))
